...
%   i and j
    ...
    cot1 = -dot(e3, e2, 2) ./ veclen(cross(e3, e2, 2));
    cot2 = -dot(e3, e1, 2) ./ veclen(cross(e3, e1, 2));
    cot3 = -dot(e1, e2, 2) ./ veclen(cross(e1, e2, 2));
    
    % constructors for sparse
    ii = [idx1, idx2, idx2, idx3, idx3, idx1];
    jj = [idx2, idx1, idx3, idx2, idx1, idx3];
    v  = [cot3, cot3, cot1, cot1, cot2, cot2];
    
    cotMat = sparse(ii, jj, v);
    
    % make diagonal elements non zero
    diagElem = sum(cotMat, 2);
    idx = 1:numVertices;
    diagSparse = sparse(idx, idx, diagElem);
    
    cotMat = cotMat - diagSparse;
    ...
    triArea = 0.5 * veclen(cross(e1, e2, 2));
